...
required: true, message: '请输入你的昵称', trigger: 'blur' }]">
        ...
        <el-form-item class="input-prepend restyle no-radius"
                      prop="email"
                      :rules="[{ required: true, message: '请输入邮箱', trigger: 'blur' },{trigger: 'blur'}]">
          <!-- :rules="[{ required: true, message: '请输入邮箱', trigger: 'blur' },{validator:
  ...
  methods: {
    getCodeFun () {
      ...
      this.$refs.userForm.validateField('email', (errMsg) => {
        if (errMsg == '') {
          registerApi.getEmail(this.params).then(res => {
            this.sending = false;
            this.timeDown();
          });
        }
