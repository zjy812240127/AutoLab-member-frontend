              ...
              <li v-for="item in data.list"
                ...
                <section class="i-teach-wrap">
                  ...
                  <div class="mt10 hLh30 txtOf tac">
                    <a :href="'/activity/'+item.id"
                       :title="item.title"
                       class="fsize18 c-666">{{ item.title }}</a>
                  </div>
                  <!-- <div class="hLh30 txtOf tac">
                    <span class="fsize14 c-999">{{ item.career }}</span>
                  </div> -->
                  <div class="mt15 i-q-txt">
                    <p class="c-999 f-fA">{{ item.description }}</p>
                  </div>
                </section>
              </li>
            </ul>
...
export default {
  ...
  asyncData ({ params, error }) {
    return activity.getPageList(1, 8).then(response => {
  ...
  methods: {
    ...
    gotoPage (page) {
      activity.getPageList(page, 8).then(response => {
        this.data = response.data.data
      })
    }
  }
